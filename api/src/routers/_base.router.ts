...
import { NetsuiteClass } from '../core/netsuite-auth.class'
...
export class _baseRouter {
  ...
  public async send_netsuite_req(script_id: number, payload:any) {
   // (new NetsuiteClass()).getNewToken();
    // load oauth2 token
    let token: any = (await ServerData.findOne({ where: { key: 'ns-token'}})).data;

  //  console.log(token)

    // refresh token if it's dead =============================================
    if (isJwtExpired(token.access_token)) { 
        
      try {
        let tokenClass = new NetsuiteClass();
        token = await tokenClass.getNewToken();
      } catch (error) {
        console.log(error)
      }
    ...
    const script_url: string = `https://4891605.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=${script_id}&deploy=1`;
    return axios.post(script_url, payload, { headers });
  }
}