...
export class UserClass extends _baseClass {
	 ...
	 */
	async create_user({ fullname, username, password, customer_id, email }: User) {
		let new_password: string = hashSync(password, genSaltSync())
		...
		try {
			...
			const u: User = await User.create(
				{ fullname, customer_id, username, email, password: new_password },
				{ transaction: t }
			);
